...
for pr in ${pr_list}; do
  ...
  if [[ -z $(ls -A "${pr_dir}/RUNTESTS/EXPDIR") ]] ; then
    ...
    "${HOMEgfs}/dev/ci/scripts/utils/pr_list_database.py" --remove_pr "${pr}" --dbfile "${pr_list_dbfile}"
    ...
    if [[ -n "${weekly_labels}" ]]; then
      num_platforms=$(find "${HOMEgfs}/dev/ci/platforms" -type f -name "config.*" | wc -l)
      passed=0
      for platforms in "${HOMEgfs}"/dev/ci/platforms/config.*; do
